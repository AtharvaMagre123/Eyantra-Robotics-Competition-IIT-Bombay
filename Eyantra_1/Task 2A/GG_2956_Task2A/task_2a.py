    ...
    * ArUco_corners = 
    ...
    for ids, corners in zip(marker_IDs, marker_corners):
        ...
        if y[1] == max(y):
            ...
            midy = (y[0]+y[1])/2
            slope = (Y-midy)/(X-midx)
            angle = -(90+float(57.3*math.atan(slope)))
            ArUco_details_dict[z[0]] = [[int(X), int(Y)], int(angle)]
            z = z[1:len(marker_IDs)]
        else:
            midx = (x[0]+x[1])/2
            midy = (y[0]+y[1])/2
            slope = (Y-midy)/(X-midx)
...
if __name__ == "__main__":
    ...
    img_dir_path = "F:/Eyantra_1/Task 2A/Task_2A_files/Task_2A_files/public_test_cases/"
